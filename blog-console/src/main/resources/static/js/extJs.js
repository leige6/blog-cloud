...
$.ajaxSetup({
    ...
    error : function(XMLHttpRequest, textStatus, errorThrown) {
        try {
            ...
            parent.$.messager.alert('提示', '请求超时', 'info');
        } catch (e) {
    ...
    complete: function(XMLHttpRequest, textStatus, xhr) {
        ...
        if(oauthstatus == '401'){
            self.location.href = getBasePath() + "/login.shtml";
            return;
        }
        var duplicateSubmit=XMLHttpRequest.getResponseHeader("duplicateSubmit");
        if(duplicateSubmit == '505'){
            parent.$.messager.progress('close');
            parent.$.messager.alert('提示', '请不要重复提交', 'info');
            return;
        }
    }
});
