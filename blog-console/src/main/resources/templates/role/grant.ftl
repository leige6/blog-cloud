    ...
    $(function() {
        resourceTree = $('#resourceTree').tree({
            ...
            onLoadSuccess : function(node, data) {
                progressLoad();
                $.post( '/role/findResourceIdListByRoleId.shtml', {
                    id : '${rid!}'
                }, function(result) {
                    var ids;
                    if (result.state == true && result.data != undefined) {
    ...
    <div data-options="region:'west'" title="系统资源" style="width: 300px; padding: 1px;">
        <div class="well well-small">
            <form id="roleGrantForm" method="post">
                <input name="id" type="hidden"  value="${rid!}" readonly="readonly">
